    ...
    <div className='w-full'>
      <div className='bg-black text-white h-16 px-[6%] flex items-center justify-between relative'>
        <h1 className='text-xl font-bold cursor-pointer' onClick={() => {
          if ((location.pathname.includes("/book-ride") || location.pathname.includes("/ride") || location.pathname.includes("/history-page") || location.pathname.includes("/profile")) && queryClient.getQueryData(['user'])?.userType === "Parent") {
            navigate("/book-ride");
            return
          }
          if ((location.pathname.includes("/book-ride") || location.pathname.includes("/ride") || location.pathname.includes("/history-page") || location.pathname.includes("/profile")) && queryClient.getQueryData(['user'])?.userType === "Driver") {
            navigate("/rides");
            return
          }
          navigate("/")
        ...
        {(location.pathname.includes("/book-ride") || location.pathname.includes("/ride") || location.pathname.includes("/history-page") || location.pathname.includes("/profile")) &&
          <div className='flex items-center justify-center gap-2 relative group cursor-pointer py-2'>
            ...
            <div className='select-none'>
              <h1 className='text-[16px] xsm:hidden'>{user?.fullName}</h1>
            </div>
